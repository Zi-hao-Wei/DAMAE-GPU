...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        self.feature_pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, decoder_embed_dim),
                                              requires_grad=False)
    ...
    def initialize_weights(self):
        ...
        feature_decoder_pos_embed = get_2d_sincos_pos_embed(self.decoder_pos_embed.shape[-1],
                                                    int(self.patch_embed.num_patches ** .5), cls_token=True)
    ...
    def feature_forward_decoder(self, x, ids_restore, need_mask=True, global_pool=False):
        # embed tokens
        cls = None
        p = None
        ...
        xs=[]
        # apply Transformer blocks
        for i, blk in enumerate(self.feature_decoder_blocks):
            if torch.isnan(x).any():
                print("break at block", i)
            x = blk(x)
    ...
    def forward(self, imgs, smaller_imgs, mask_ratio=0.75, device=None, double_loss=False, epoch=0):
        xs, latent, mask, ids_restore, _ = self.forward_encoder(imgs, mask_ratio, None)
        pred, _, _ = self.forward_decoder(latent, ids_restore, True, False)  # [N, L, p*p*3]
        _, z, _ = self.feature_forward_decoder(latent, ids_restore, True, False) 
        loss = self.forward_loss(imgs, pred, mask)
        if double_loss:
            _, latent_smaller, _, _, _ = self.forward_encoder(smaller_imgs, 0, None)
            _, z_, _ = self.feature_forward_decoder(latent_smaller, ids_restore, False, False)
...
mae_vit_base_patch16 = mae_vit_base_patch16_dec512d4b  # decoder: 512 dim, 8 blocks
