...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, ([samples, smaller_samples, sep], _) in enumerate(
        ...
        with torch.cuda.amp.autocast():
            total_loss, sim_loss, loss_mae, loss_sr, _, _, _, = model(samples, smaller_samples, mask_ratio=args.mask_ratio,
                                                      device=device, double_loss=True, epoch=epoch)
        total_loss_value, sim_loss_value, loss_mae_value, loss_sr_value = total_loss.item(), sim_loss.item(), loss_mae.item(),loss_sr.item()
